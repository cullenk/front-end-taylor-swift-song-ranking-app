...
@Component({
  selector: 'app-lover-ranking',
  standalone: true,
  imports: [DragDropModule, CommonModule],
  templateUrl: './lover-ranking.component.html',
  styleUrls: ['./lover-ranking.component.scss']
})
export class LoverRankingComponent implements OnInit {
  rankings: Ranking[] = [];
  ...
  loadData() {
    this.albumService.getAlbumBySong('Cruel Summer').subscribe(
      (album: Album) => {
        this.album = album;
  ...
  loadRankings() {
    this.rankingsService.getUserRankings().subscribe(
      (rankings: Rankings) => {
        if (rankings && rankings.albumRankings && rankings.albumRankings['lover']) {
          this.rankings = rankings.albumRankings['lover'].sort((a, b) => a.rank - b.rank);
  ...
  updateRankings() {
    ...
    this.rankingsService.updateRanking('lover', newRankings).subscribe(
