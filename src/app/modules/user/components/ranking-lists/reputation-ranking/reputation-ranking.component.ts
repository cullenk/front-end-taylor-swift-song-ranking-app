...
@Component({
  ...
  templateUrl: './reputation-ranking.component.html',
  styleUrls: ['./reputation-ranking.component.scss']
})
export class ReputationRankingComponent implements OnInit {
  rankings: Ranking[] = [];
  ...
  loadData() {
    this.albumService.getAlbumBySong('I Did Something Bad').subscribe(
      (album: Album) => {
        this.album = album;
  ...
  loadRankings() {
    this.rankingsService.getUserRankings().subscribe(
      (rankings: Rankings) => {
        if (rankings && rankings.albumRankings && rankings.albumRankings['reputation']) {
          this.rankings = rankings.albumRankings['reputation'].sort((a, b) => a.rank - b.rank);
  ...
  updateRankings() {
    ...
    this.rankingsService.updateRanking('reputation', newRankings).subscribe(
