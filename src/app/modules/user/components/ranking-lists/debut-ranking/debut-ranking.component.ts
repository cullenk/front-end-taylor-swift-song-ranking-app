...
@Component({
  ...
  templateUrl: './debut-ranking.component.html',
  styleUrls: ['./debut-ranking.component.scss']
})
export class DebutRankingComponent implements OnInit {
  rankings: Ranking[] = [];
  songs: Song[] = [];
  ...
  loadAlbumData() {
    this.albumService.getAlbumByTitle('Taylor Swift').subscribe(
      (album: Album) => {
  ...
  loadRankings() {
    this.rankingsService.getUserRankings().subscribe(
      (rankings: Rankings) => {
        if (rankings && rankings.albumRankings && rankings.albumRankings['taylorSwift']) {
          this.rankings = rankings.albumRankings['taylorSwift'].sort((a, b) => a.rank - b.rank);
  ...
  updateRankings() {
    ...
    this.rankingsService.updateRanking('taylorSwift', newRankings).subscribe(
