...
@Component({
  selector: 'app-nineteen-eighty-nine-ranking',
  standalone: true,
  imports: [DragDropModule, CommonModule],
  templateUrl: './nineteen-eighty-nine-ranking.component.html',
  styleUrls: ['./nineteen-eighty-nine-ranking.component.scss']
})
export class NineteenEightyNineRankingComponent implements OnInit {
  rankings: Ranking[] = [];
  songs: Song[] = [];
  ...
  loadAlbumData() {
    this.albumService.getAlbumByTitle("1989 (Taylor's Version)").subscribe(
      (album: Album) => {
  ...
  loadRankings() {
    this.rankingsService.getUserRankings().subscribe(
      (rankings: Rankings) => {
        if (rankings && rankings.albumRankings && rankings.albumRankings['nineteenEightyNine']) {
          this.rankings = rankings.albumRankings['nineteenEightyNine'].sort((a, b) => a.rank - b.rank);
  ...
  updateRankings() {
    ...
    this.rankingsService.updateRanking('nineteenEightyNine', newRankings).subscribe(
